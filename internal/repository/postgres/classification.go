...
func (r *ClassificationRepository) ListUnclassifiedDevicesWithPagination(ctx context.Context, limit, offset int) ([]string, error) {
	query := `
		SELECT d.id 
		FROM devices d 
		LEFT JOIN device_classifications dc ON d.id = dc.device_id 
...
func (r *ClassificationRepository) CountUnclassifiedDevices(ctx context.Context) (int, error) {
	query := `
		SELECT COUNT(d.id)
		FROM devices d 
		LEFT JOIN device_classifications dc ON d.id = dc.device_id 
...
func (r *ClassificationRepository) SaveDeviceClassification(ctx context.Context, c classification.DeviceClassification) error {
	// UUIDが設定されていない場合は生成
	if c.ID == "" {
