...
// ConnectionClassification represents classified connections for a device
type ConnectionClassification struct {
	Uplinks   []ConnectionInfo `json:"uplinks"`   // 上位階層への接続
	Downlinks []ConnectionInfo `json:"downlinks"` // 下位階層への接続
	Peers     []ConnectionInfo `json:"peers"`     // 同一階層への接続
}
